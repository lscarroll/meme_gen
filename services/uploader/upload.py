...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("../../data/youtube_upload.log"),
...
def get_authenticated_service():
    ...
    if os.path.exists('../../services/uploader/token.json'):
        try:
            with open('../../services/uploader/token.json', 'r') as token:
        ...
        if not creds:
            try:
                flow = google_auth_oauthlib.flow.InstalledAppFlow.from_client_secrets_file(
                    '../../services/uploader/client_secrets.json', SCOPES)
                ...
                with open('../../services/uploader/token.json', 'w') as token:
...
def generate_thumbnail(video_path, output_path):
    ...
    try:
        ...
        # Try to get a frame from 25% into the video, preserving aspect ratio
        cmd = [
            "ffmpeg", 
            "-i", video_path, 
            "-ss", "00:00:30",  # 30 seconds in
            "-vframes", "1", 
            "-vf", "scale=1280:-1",  # Scale to 1280 width, auto height to preserve aspect ratio
...
def main():
    try:
        ...
        # Directories (adjusted for Docker container paths)
        ...
        for video_info in all_videos:
            ...
            # Upload the original video with thumbnail (no audio processing)
            video_id = upload_video(youtube, video_path, thumbnail_path, video_type=video_type)
            ...
            if video_id:
                ...
                # No cleanup needed since we're uploading original video directly
